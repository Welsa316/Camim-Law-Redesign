...
**Superseded on 2026-09-25.** The opening frame is a slideshow of three
photographs now, on the client's instruction: he liked the sister firm's
cycling hero and not the picture of himself that was in ours. The composite
described below is no longer on any page, so the generated-margin disclosure
no longer describes anything the public can see. `juan-campos-hero-wide.jpg`
and `-tall.jpg` remain on disk; nothing references them. The paragraph is kept
because the file is kept — if either is ever put back, this is what it is.
...
1. **The opening frame cycles three photographs**, on the client's instruction of 2026-09-25 — the sister firm's design, not its pictures. They are Orlando's own skyline over Lake Eola, a neoclassical courthouse, and an empty departure gate: the city, the kind of building these cases are decided in, and the journey. All three are Unsplash License, checked on each photo page that day and recorded in `docs/sources/images.json`. Nobody in them is staged as a client, and none of them is a flag, a Statue of Liberty, a gavel or a set of scales.

   The sister firm's own slideshow was examined and **not** copied: its five slides are a US flag, the Statue of Liberty, a photograph of Juan, a synthetic stock image of a distressed man captioned "Helping survivors", and a stock wedding couple captioned "Visas for spouses". Four are on this project's exclusion list, and the fourth is the kind of emotionally manipulative imagery the advertising rules reach — a person in distress, not a client, used to advertise legal services. **The firm should know this is on camulaw.com, whatever it decides for this site.**

   The seated Google Business Profile portrait that used to carry this frame is no longer on any page.
