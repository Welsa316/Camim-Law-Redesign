...
export const ATTORNEY = {
   ...
   */
  /**
   * Two frames from the same original.
   *
   * `tall` is the photograph, cut at source y 9-91% so the whole seated pose
   * and the chair are in frame. It serves portrait viewports.
   *
   * `wide` serves landscape viewports and is a composite. The centre of it,
   * from 22% to 75% of its width, is the photograph at full resolution, with
   * the real ceiling and pendant above his head. Beyond a feathered band on
   * each side, the room is a generated extension the client produced with
   * ChatGPT's image model from this photograph and supplied on 2026-09-09.
   * The generated file also re-drew him, so nothing of its figure is used:
   * the photograph is laid over it and the generated pixels are only ever
   * floor, wall, chairs and a sofa. There are no people, text or marks in
   * the generated region. This is disclosed in docs/CONTENT_REVIEW.md.
   */
  hero: v(
    {
      wide: { src: "/img/juan-campos-hero-wide.jpg", width: 2800, height: 1725 },
      tall: { src: "/img/juan-campos-hero-tall.jpg", width: 1600, height: 1967 },
    },
    "Google Business Profile, Campos Immigration Law Firm LLC; margins of `wide` generated by the client, 2026-09-09",
  ),
  /**
   * The opening frame cycles three photographs instead of carrying one
   * portrait, on the client's instruction of 2026-09-25: he liked the sister
   * firm's slideshow and not the picture of himself that was here. The
   * subjects are his city, the kind of building these cases are decided in,
   * and a departure gate — America and the work, with nobody staged as a
   * client. Licences in docs/sources/images.json.
   */
  heroSlides: v(
    [
      { src: "/img/hero/orlando-night.jpg", focus: "50% 58%" },
      { src: "/img/hero/courthouse.jpg", focus: "50% 42%" },
      { src: "/img/hero/airport-gate.jpg", focus: "50% 50%" },
    ],
    "Unsplash, three photographs under the Unsplash License; selected here on the client's instruction of 2026-09-25",
  ),
  seated: v(
...
export const MEDIA = [
  ...
  },
  {
    ...
    programme: "Noticias",
    chyron: {
      es: "USCIS está poniendo mayor atención a trámites de ciudadanía por matrimonio",
      en: "USCIS is paying closer attention to marriage-based citizenship applications",
    },
    /**
     * Cropped above the broadcast's sports ticker, which carries a different
     * law firm's name as its sponsor. The interview, the chyron and the
     * network mark are all in the kept region. Source: the sister firm's
     * press folder, supplied by the client.
     */
    still: "/img/media-univision.jpg",
    width: 906,
    height: 434,
    verified: true,
  },
...
/**
 * A Google Maps embed for the office, built from the verified name and
 * address rather than from a pasted embed code. The keyless `output=embed`
 * form has no API key to leak and resolves the query to the Business Profile
 * listing when the name and address match, which they do. `ORG.mapsUrl` is
 * still unverified, so nothing links to a profile URL; this only draws the map.
 */
export const mapEmbedUrl = (): string => {
  const a = ORG.address.value;
  const q = `${ORG.legalName.value}, ${a.street}, ${a.locality}, ${a.region} ${a.postalCode}`;
